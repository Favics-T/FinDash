  ...
  return (
    <div className="relative">
      ...
      <div className="relative">
        <SearchIcon
          size={14}
          className={`pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 transition-colors duration-200 ${focused ? 'text-[var(--color-primary)]' : 'text-[var(--color-on-surface-variant)]'}`}
        />
        <input
          ref={inputRef}
          type="text"
          ...
          className={`rounded-[10px] border pl-9 text-[13px] outline-none transition-all duration-200 font-sans text-[var(--color-on-surface)] py-[7px] ${query ? 'pr-9' : 'pr-3.5'} ${focused ? 'w-[340px] border-[var(--color-primary)] bg-[rgba(0,212,232,0.04)] shadow-[0_0_0_3px_rgba(0,212,232,0.1)]' : 'w-[260px] border-[rgba(42,42,56,0.9)] bg-[rgba(255,255,255,0.04)]'}`}
        />
        {query && (
          <button
            onClick={() => { setQuery(''); inputRef.current?.focus(); }}
            className="absolute right-[10px] top-1/2 -translate-y-1/2 flex cursor-pointer border-none bg-transparent p-[2px] text-[var(--color-on-surface-variant)]"
          >
            <X size={13} />
          </button>
        )}
      ...
      {showDropdown && (
        ...
        >
          <div className="border-b border-[rgba(42,42,56,0.6)] px-[14px] pt-2 pb-[6px] text-[9px] font-bold tracking-[0.14em] text-[var(--color-on-surface-variant)]">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-[10px]">
                    ...
                    <div>
                      <p className="text-[13px] font-semibold leading-[1.2] text-[var(--color-on-surface)]">{coin.name}</p>
                  ...
                  <div className="text-right">
                    <p className="font-mono text-[13px] font-bold text-[var(--color-on-surface)]">
                      {formatCurrency(coin.current_price)}
                    </p>
                    <p className={`flex items-center justify-end gap-[2px] font-mono text-[10px] font-semibold ${up ? 'text-[var(--color-secondary)]' : 'text-[var(--color-error)]'}`}>
