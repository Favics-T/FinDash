...
import { ArrowLeftRight, TrendingUp, TrendingDown, Info, Plus, Download } from 'lucide-react';
...
// Inner compare content rendered for Pro users
const CompareContent = () => {
  ...
  return (
    <>
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-lg gap-lg">
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              <h2 className="font-h1 text-3xl font-bold text-on-surface tracking-tight">Asset Comparison</h2>
              <ProBadge />
            </div>
            <p className="text-on-surface-variant font-body-md max-w-2xl">
              Side-by-side performance analysis and fundamental health metrics. Compare up to 3 assets simultaneously.
            </p>
          </div>
          ...
          <div className="flex items-center gap-sm">
            ...
            >
              <Download size={16} />
              Export Sync
              {!isPro && <ProBadge style={{ fontSize: 7 }} />}
            </Button>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-outline-variant/10">
                {metrics.map((metric, i) => (
                  <tr key={i} className="hover:bg-surface-bright/30 transition-colors group">
                    ...
                    {selectedIds.length < 3 && <td className="p-lg" />}
                  </tr>
                ))}
            ...
            <CardContent>
              <div className="space-y-md">
                ...
                <div className="w-full bg-surface-container-highest h-2 rounded-full overflow-hidden">
                  <div className="bg-secondary h-full" style={{ width: '92%' }} />
                </div>
                <p className="text-xs text-on-surface-variant">Assets are moving in near-perfect lockstep. Diversification benefit is currently minimal.</p>
              </div>
            </CardContent>
          </Card>
      ...
      <UpgradeModal isOpen={modalOpen} onClose={() => setModalOpen(false)} featureLabel="Export Sync" />
    </>
  );
};
...
// ─────────────────────────────────────────────────────────────────────────────
const Compare = () => (
  <ProGate featureKey="assetComparison" featureLabel="Asset Comparison" blur={8}>
    <CompareContent />
  </ProGate>
);
